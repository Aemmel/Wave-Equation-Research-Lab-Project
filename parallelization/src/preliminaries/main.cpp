...
void fill_matrix_with_vals(myownmatrix &m, const list &x, const list &y, const list& z, fOfxyz func)
{
...
myownmatrix create_matrix_with_vals(const list &x, const list &y, const list &z, fOfxyz func)
{
    myownmatrix m(x.size(), y.size(), z.size());
...
myownmatrix first_mixed_deriv_2nd_order(const myownmatrix& u, unsigned short c1, unsigned short c2, double h1, double h2)
{
    if (c1 == c2) {
...
void QB()
{
    ...
    myownmatrix mat = create_matrix_with_vals(x, y, z, foo);
    myownmatrix mat_xy_a = create_matrix_with_vals(x, y, z, foo_xy);
    myownmatrix mat_xy_n = first_mixed_deriv_2nd_order(mat, 1, 2, hx, hy);
    myownmatrix mat_yz_a = create_matrix_with_vals(x, y, z, foo_yz);
    myownmatrix mat_yz_n = first_mixed_deriv_2nd_order(mat, 2, 3, hy, hz);

    myownmatrix mat_xy_diff(mat_xy_a.get_shape());
    myownmatrix mat_yz_diff(mat_yz_a.get_shape());

    myownmatrix t_1 = mat_xy_a - mat_xy_n;
    myownmatrix t_2 = mat_yz_a - mat_yz_n;
