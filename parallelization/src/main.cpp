...
matrix_t we_func_full(const matrix_t& m, const double dx)
{
    matrix_t deriv = matrix_t(m.rows(), m.cols());
    second_deriv_4th_order(deriv, m, dx, DIV_AX::X, 2);

    // TODO FIX
    deriv.unaryExpr([](element_t x){
        return x.imag() + 1i * 1. * x.real();
    });
}
...
void wave_eq_1D_periodic_bc_basic()
{
    ...
    // // test output
    // for (ind_t i = 0; i < cols + 2*num_ghost; ++i) {
    //     cout << field_matrix(num_ghost, i).real() << endl;
    // }
    ...
    for (double t = start_time; t < stop_time; t += dt) {
        ...
        second_deriv_4th_order(field_matrix_deriv, field_matrix, dx, DIV_AX::X, num_ghost);
